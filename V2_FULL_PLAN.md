...
**Phase B2 — supply chain (1 d)**
- `uv.lock` / `requirements.txt` with hashes; `--require-hashes` install documented.
- CycloneDX SBOM generated in CI, attached to the release.
- GPG-signed tags; cosign-signed images; SLSA provenance from public Actions; install-by-digest in the README.
- Semgrep, Bandit, `pip-audit`, Trivy in public CI with linkable runs.
...
- `corpus/` planting pipeline: insert invariants into templated documents at declared locations.
- Rewrite evaluators 4–14 to the recipes in §8.2 — inverted checks, set membership, adjacency by sentence unit.
- Injection payloads by side effect (§3.3), not by manipulation judgment.
- **Acceptance:** every Tier 1 evaluator shipped at this point — 15 of the 18 specified, with #18 arriving in Phase G to make 16 — references no model at scoring time, asserted by a test that the Tier 1 scorer imports nothing from `tier2/`; plant collision test over 10,000 generations passes.
