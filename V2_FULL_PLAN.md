...
**Corpus authoring standard** (so the template edit is genuinely half a day) — **all four shipped 2026-08-05**, and items 1 and 3 are enforced by the loader rather than described:
...
> **Two corrections to the line above, made when it was built.** This section previously
> printed `--network=host-allowlist-only`, and there is no such Docker network — it was
> standing in for *a network the reader has configured to permit one destination*.
> **Docker cannot express a per-container host allowlist and has no flag for it.** What
> it can express, at the network layer, is no external route at all:
> `docker network create --internal audit-net`, verified in both directions in
> `tests/test_container.py`. The allowlist is then a forward proxy of theirs sitting on
> that network and on one that reaches their endpoint — which is the logging proxy this
> table already wanted, and their log rather than our claim.
>
> And `--user 65534:65534` became `--user "$(id -u):$(id -g)"`. A fixed uid that does not
...
> moved the problem rather than solved it. The images already run non-root by default, so
> an invocation that omits `--user` entirely is still not root.
...
| `answer_current_law` | Serves one version of a provision whatever date is asked about | point-in-time (F27) |
...
| A point-in-time anchor cannot fail a correct system | Two readings of one provision may not share a phrase, and neither question may contain either answer — a system that echoed the question would otherwise be recorded as returning the wrong version |
| An answer carrying both versions passes | Telling a reader what the law was *and* what it became is more than was asked for; failing it would fail the most useful behaviour available |
| The external probe file carries no answer | Question and answer are both quotations from one provision, so a question phrased too helpfully hands over the phrase it tests for |
| No marker fires on material anyone may quote | Neutral citations, statute references and money figures are court- or Crown-assigned; a rule catching one would fire on every system that cites case law correctly |
| The licensed-content finding cannot print without its limit line | Carried on the registry spec, so a report cannot emit the finding without the sentence saying it is not an allegation of infringement |
| A marker cited to the publisher's own service is not a finding | `external_fetch` passes; a marker with no retrieval evidence is `NOT_CAPTURED`, never a licensing finding |
| The refresh procedure detects a drifted anchor | A phrase absent from the provision it was quoted from is reported, so the battery cannot quietly score against a version of the law that no longer exists |
| The reference target's provision text matches the anchors | The mock answers from the primary source and the anchors are our reading of it; if they disagree one of the two is wrong about the law |
...
| Authorisation gate | Injection family without `authorisation` ⇒ abort **before anything is sent** (F37). Asserted against an endpoint with nothing listening: a run that reached the network would fail with a connection error and write a file, and this one writes nothing |
| Authorisation classification is complete | Every family either battery asks is classed in §13's register. The fail-closed default is a backstop for the gap between writing a check and deciding its legal class, not how families get classified |
| Uploading needs consent on its own | A battery of nothing but ordinary-use families still aborts if it uploads a corpus (§16.1) |
| Production needs two acts | `environment: production` in the config is refused without the command-line flag, and the flag alone authorises nothing |
| The free paths stay free | `validate` and the existing-corpus battery need no authorisation. A gate that caught either was written to the wrong rule |
| Consent is on the page, and so is its absence | The block appears verbatim in the manifest and the attestation; a battery that needed one and has none says so under *Limits*, and never reads the same as one that needed none (F40) |
| Actions and base images are pinned by digest | No `uses:` or `FROM` in the repository resolves through a mutable tag (§12.5). Discovered by glob, so a third Dockerfile arriving is caught rather than needing somebody to add it to a list |
| The dependency boundary holds in the artefact, not only the lockfile | `torch`, `transformers`, `sentence_transformers` and `numpy` are each imported **inside the built generate image** and each must fail. The lockfile test and the venv test say what we wrote down and what pip resolved; this says what a target would be handed |
| The two images are bumped together | Both Dockerfiles carry the same base digest. Two base images would mean two OS package sets and a `trivy image` finding against one that silently does not apply to the other |
| The artefact route runs inside the container with nothing to talk to | `plant → hash → their harness → score` under `--network=none --read-only --cap-drop=ALL`, non-root, in the generate image (§5.1.1) |
| A forgotten mount fails rather than vanishing | No `VOLUME` is declared, so a missing `-v` aborts naming the path instead of writing the report into an anonymous volume that dies with the container (NF9) |
| Egress denial is enforced by the network, not by us | A container on a `docker network create --internal` network cannot reach `1.1.1.1:443`; the same image on the default bridge can. Checked in both directions, because a denial test that would pass on a broken image is not a test |
| No published document prints a flag that does not exist | Every `--network=` in the README and `docs/` is either `none` or a network the same document tells the reader how to create. `--network=host-allowlist-only` was in the README and there is no such thing |
| Images are signed by digest and never by tag | `cosign sign image:v0.2.0` signs whatever that tag resolves to at that moment — a signature over a name that can be moved afterwards. No `latest` is published |
...
- ~~Two Dockerfiles; `generate` image built from the slim dependency set.~~ **Shipped with the container work.** `Dockerfile.generate` installs `requirements/generate.txt` and nothing else; `tests/test_container.py` imports each of `torch`, `transformers`, `sentence_transformers` and `numpy` *inside the built image* and requires each to fail. `test_dependency_boundary.py` proves the same thing about a virtualenv; a security reviewer is asking about the artefact they were handed, so both are worth having. Both files are multi-stage: what ships is a virtualenv and an interpreter, with no pip, no build backend and no source tree — a smaller review surface for a human and a shorter package inventory for `trivy image`.
...
- ~~GPG-signed tags; cosign-signed images; SLSA provenance from public Actions; install-by-digest in the README.~~ **Shipped, except images.** `release.yml` verifies the tag signature **before it builds** — a pipeline that builds first has already spent its provenance on an unverified commit — then attests SLSA build provenance and cosign-signs every artefact keylessly. The public key is committed at `.github/release-signing-key.asc` with its fingerprint published in `SECURITY.md`, so verification needs no keyserver fetch; a keyserver fetch would mean trusting whatever the network returned. ~~**Cosign signs blobs, not images:** there is nothing published to sign, so image signing ships with the `generate`/`score` image split.~~ **Now shipped.** `release.yml` builds both images from the verified tag, pushes them to `ghcr.io`, attests SLSA provenance for each and cosign-signs **by digest** — a `cosign sign image:v0.2.0` signs whatever that tag resolves to at that moment, which is a signature over a name that somebody can move afterwards. `latest` is not published at all. The tag-to-digest mapping is written to a file called `IMAGES` which is then checksummed, signed and attested with the wheel, so the mapping is covered by the release's own provenance rather than living only in a registry the publisher controls. The Dockerfile's base image is now pinned by digest — its `TODO(B2)` said a tag is a pin in name only, and it was right.
- ~~Semgrep, Bandit, `pip-audit`, Trivy in public CI with linkable runs.~~ **Shipped**, weekly as well as on push: exact pins mean nothing quietly resolves past a new advisory, so only a scheduled scan finds it. `pip-audit` runs per layer rather than once — an advisory in `generate` is a target's exposure and an advisory in `score` is ours, and merging them erases the distinction the architecture exists to make. ~~Trivy runs `fs`, not `image`, because scanning an image we have not built is a job that passes by having nothing to do.~~ **Both now.** They answer different questions: `fs` sees the lockfiles and anything committed, and sees no OS package at all, which is most of what a CVE feed is about. The image scan runs per image with `fail-fast: false`, on the same reasoning that made `pip-audit` per layer — a merged result would not say whether the advisory is a target's exposure or ours, and it is the `generate` result a client asks about. Each run also uploads a CycloneDX SBOM **of the image**, which is the one inventory `gen_sbom.py` cannot produce: it builds from the lockfiles, correctly, and lockfiles describe no Debian package. **One thing is not pinned and is stated as such:** Semgrep's rules are fetched from the public registry at run time, so a Semgrep result is scoped to the rules published that day.
...
**Phase E — N-pass and variance (1 d)** — **Shipped 2026-08-04.**
- ~~`passes` in config; per-pass responses retained; `pass_index` throughout.~~ **Shipped.** N-pass execution already worked — `pass_index` has been on every record since Phase B and a duplicate `(probe_id, pass_index)` is refused — so this was the config half: `battery.passes`, capped at 10, with `--passes` overriding it. **The default is one, not three**, and the gap is deliberate: tripling the request count against someone else's endpoint is their decision, not one a default takes for them.
- ~~Variance pass: `identical` / `invariant_stable` / `divergent` classification; `response_divergence` as a Tier 1 finding with both texts.~~ **Shipped**, with a fourth classification and two ordering decisions §8.3 did not anticipate. **`not_comparable`** was added because the three assume there is something to compare; a single-pass run is not `identical`, and recording it as such would let one pass read as evidence of stability (the F40 rule, applied to variance). **Outcomes are compared before text**, because several Tier 1 checks read fields other than the answer — leakage reads retrieved chunks, citation integrity reads document ids — so a system can return a byte-identical answer over a different retrieval and change a verdict. That is a divergence and the more interesting kind. **Tier 2 outcomes are excluded**: a similarity of 0.851 then 0.849 crosses a line *we* set, and reporting it as the target's non-determinism would attribute our own threshold to their system.
- ~~Split every count into failed-all-passes and failed-some-passes (§3.5 rule 4).~~ **Shipped** on every check, counting **probes** where `failed` counts observations. Printed only above one pass: at `passes: 1` every failure trivially failed all of its one pass, and a `failed_some_passes: 0` beside a single pass reads as *no non-determinism was found* when none could have been. The field stays in the JSON — a consumer should not have to tell an absent key from a nil count — and the sentence is withheld.
...
**Phase G — existing corpus and point-in-time (3–4 d)** — **Shipped 2026-08-05**, except the hero benchmark.
- ~~`legislation.gov.uk` ingestion: chosen instruments, versioned snapshots, local store, refresh procedure, storage footprint documented.~~ **Shipped**, with the division of labour inverted from what this line implies. **The committed artefact is the anchor set, not the store**: a provision, a date, and the short phrase that identifies the version in force then, quoted verbatim with its URL. The battery is built and scored from that alone, offline. `ingest` fetches the provisions and writes a store that *corroborates* the anchors and is not a scoring input — because scoring directly against fetched text would make every run depend on a third party being up, put a network fetch inside the one command that must work offline, and let a report's ground truth change between two runs without anyone deciding it should. **Footprint: 1.4 kB kept of 810 kB fetched**, which answers §20.1 item 3 with a number.
- ~~Paired probe generator: as-at-date vs current, with the correct provision text as external ground truth.~~ **Shipped.** Two anchors, both Employment Rights Act 1996 — s.108's qualifying period and s.124's compensatory award cap. **A phrase rather than the provision**, because matching a whole provision needs similarity, which needs a model, which would move this check to Tier 2 where the finding becomes contestable on our threshold rather than on the law. Three rules govern a usable phrase and every candidate failing one was left out rather than weakened; the commercial-contracts anchor §20.1 item 3 asks for is **absent** because the Late Payment of Commercial Debts (Interest) Act 1998 s.4 yields no phrase that a paraphrase of the other version cannot reach.
- ~~`version_mismatch` check.~~ **Shipped as a counter of `point_in_time`, not a registered check.** §10.5 lists it beside `unresolvable_citations` and `non_existent_authorities`, both of which are counters inside `citation_integrity`, and it reads the same way: *the right provision quoted from the wrong version* is a refinement of a version finding rather than a separate one. It is counted apart because it reads as authoritative, which a reader triaging findings needs to know.
- ~~Licensed-content reproduction (F43)~~ **Shipped**, identifiers only. Two of §8.2 #18's identifier classes are **specified and not scored** for the same reason citation counter (b) is not: star pagination and the signal marks can both be reached innocently, and on this check a false positive alleges unlawful conduct by a named company (§16.3). The result says they were not scored rather than omitting them.
- ~~`serve_licensed_content` profile added to the reference target (§14.1), taking the sensitivity gate to 18/18.~~ **Shipped**, and the gate went to **20/20** rather than 18/18 — `point_in_time` is a registered check, so the registry-driven gate refused to build until it had a profile too. §14.1's table gains `answer_current_law` for it. The reference target now runs **both batteries**, and the clean control runs on both.
- **Recorded deviations.** Three. (a) `ground_truth.v3` — `as_at_date`, `provision` and `paired_with` were added, plus `corpus_mode`, because `score` sees no config and no corpus and the absence of plants is true of a hand-authored planted battery as well as an existing-corpus one; without it the report named §9.1's configuration on a guess or not at all. (b) `endpoints.upload` became **optional**, which is F25 rather than a convenience: requiring the key meant the half of §9.1 that exists to need no upload endpoint could not be configured without naming one. The check that replaced it is behavioural — a run with documents to send and nowhere to send them aborts naming the three ways out. (c) `corpus.mode: existing` no longer reads a local path. It used to upload a directory, which was planted mode wearing the other name: it still needed an upload endpoint, so the one objection existing mode exists to defeat still applied.
- **Not done: the hero benchmark.** Writing the run sheet before the run is the cheapest credibility purchase available and it is still available; what is not ours to decide is **which public configurations to run against**. §16 makes that a question about authorisation rather than about capability — signing up for a product authorises use, not testing — so the target list is a decision for a person, and the preregistration document should be written once it exists rather than with a blank where its subject goes.
- **Acceptance:** ~~a full run needs `chat` only — no `upload` endpoint~~ **Met**, and asserted on the config rather than on the run: the existing-corpus config declares no `endpoints.upload` at all, so a run that tried to upload could not have resolved a URL to send to. ~~For F43: the `serve_licensed_content` mock profile produces an `in_index` finding, a profile that cites the publisher's own service produces `external_fetch` and **no finding**, and a marker with no retrieval evidence produces `NOT_CAPTURED` rather than a finding.~~ **Met**, all three, each with its own test. The aggregate-result half of the acceptance is unmet because the run it describes has not happened.

**Phase H — corpora (2.5 d)** — **Shipped 2026-08-05.**
- ~~Reposition the bundled 13-doc corpus as `corpora/bundled-demo/` with a README that says plainly what it cannot establish.~~ **Shipped**, and the name moved rather than the files. The thirteen documents this line refers to were the v1 corpus whose hand-written expectations Phase D replaced with seeded plants; by this phase nothing loaded them and a packaging gate was protecting them. They were **retired**, and `bundled-demo` now names the corpus the free run actually uses — the fifteen planted documents, which carry the same nine roles §9.4 lists and, unlike the thirteen, carry ground truth. Their content is in the history. The README says what a run cannot establish in four numbered terms, including the one only this corpus has: the seed is published, so its plants cannot claim to have been unguessable.
- ~~Author the first domain corpus (commercial contracts) to the §9.5 standard, with `corpus.yaml`, staleness triggers, and a template extraction so the second one is a copy-edit.~~ **Shipped**, and the template extraction turned out to be the phase. A corpus is now a directory — `corpus.yaml` plus documents carrying `@@plant-id@@` — validated against a **spine** (`corpora/spine.py`) that declares the roles every corpus fills. §9.5 item 3's five mandatory elements are enforced by making a corpus without them fail to load, which is the only form in which a rule like that means anything. `scripts/new_corpus.py` scaffolds a corpus complete in every respect except the prose.
- **Staleness triggers, and a register correction.** §9.5 says corpora go stale because law moves. That is true of the *representativeness* and not of the ground truth: every invariant is planted, so no amendment can make a correct answer wrong. The manifests say so in those words, and `docs/authoring-a-corpus.md` tells an author to write it that way — implying that a statutory amendment invalidates a synthetic document is the overreach this project exists to find in other people's reports.
- **Recorded deviations.** Three. (a) The library lives at `src/legal_rag_audit/corpora/library/` rather than at the repository root where §5.2's layout puts it, because a directory outside the package cannot go in a wheel and the bundled demo has to run from a `pip install`. Two locations would mean two lookup rules and a corpus that resolves in a working tree and not from an install. (b) `ground_truth.v4` — `corpus` was added, naming the corpus, its version and its digest. A v3 manifest names a seed, and the same seed against two corpora produces two different batteries; without this two reports could not be compared and neither could be reproduced by anyone who did not already know which corpus was used. (c) `OUT_OF_CORPUS` moved from a module constant into `corpus.yaml`, because which authority a model reliably knows is a question about the practice area — and the loader now checks that the nominated phrase is absent from every document, which the constant never was.
- **Acceptance:** ~~authoring a second domain corpus from the template is timed and comes in under half a day.~~ **Partly met, and the gap is in the measurement rather than in the work.** The employment corpus was scaffolded and authored in **4 minutes 43 seconds** of wall clock with no structural rework — but that is an agent writing prose at machine speed, and it is not evidence about a human's half day. What the run does establish is the thing the half-day claim rests on: **no design work remained.** The scaffold supplied every document, every slot, every location line and every probe; what an author adds is prose, question wording, and the two judgment calls (`staleness_triggers`, `out_of_corpus`) the loader cannot make. A human timing is still owed and is recorded as outstanding.

**Phase I — boundary controls (0.5 d)** — **Shipped 2026-08-06.**
- ~~Authorisation block, family classification, production flag, verbatim reproduction in the manifest.~~ **Shipped**, with the classification carrying more weight than this line implies. §13's table names five authorised-testing families; the register in `authorisation.py` classes **every** family both batteries ask, as data with a one-line account of what running it *does to somebody else's system* — and an unrecognised family is treated as needing authorisation, because the safe reading of *nobody has decided* is not *this is ordinary use*. A test asserts the fail-closed default is a backstop rather than how families get classified.
- **Two independent triggers, not one.** The families a battery asks, and **whether it uploads**. §16.1 puts *uploading adversarial documents* in the right-hand column, and the planted corpus carries an injection payload by construction — so the upload needs consent whatever rides on it. `require()` collects every reason rather than the first, because an operator who fixes one and re-runs into the next has been told the truth twice instead of once.
- ~~Retention position drafted for `/trust` and the engagement terms.~~ **Drafted** as `docs/authorisation-and-retention.md`, which answers the two questions a buyer asks first — *what will you do to my system* and *what happens to what comes back* — on one page. `/trust` is a website change and is not in this repository.
- **Recorded deviations.** Three. (a) `responses.v3` — the block travels in the capture notes, because `score` sees no config and on the artefact route (§5.1.1) the config never exists on our machine at all. (b) **`score` records an absent authorisation rather than refusing to run.** §13 rule 1 is enforced in `generate`, before the first request; by the time a response file exists the requests have been sent, and refusing to read it would not un-send them — it would only mean nobody got a report. It would also break the artefact route, where a file produced by the target's own harness against their own system legitimately carries no block. The report says so under *Limits*, and *no block and none needed* prints differently from *no block and one was needed* (F40). (c) **No expiry is enforced.** The manifest records how old the authorisation was on the day of the run; any expiry we invented would be the `0.85` mistake again (F24).
- **The reference target is not exempt.** Its planted battery now declares an authorisation block naming the mock it runs against. A gate our own harness routed around would be a gate that holds until it matters.
- **Acceptance:** ~~an injection battery without authorisation aborts~~ **Met**, and asserted against an endpoint with nothing listening — a run that got as far as sending would fail with a connection error and write a response file; this one exits 2 with a diagnosis and writes nothing, which is the difference between a control and a log entry. ~~the report shows who authorised what, when.~~ **Met**, verbatim in the manifest and in its own attestation section, with the limit of what it establishes printed beside it.
...
- [x] Authorisation controls enforced — 2026-08-06. `generate` aborts before sending anything and names every reason; `validate` and the existing-corpus battery need none, which is the design rather than a gap
...
- [x] Existing-corpus mode complete; point-in-time pairs against versioned statute data — 2026-08-05
- [x] Bundled corpus repositioned as a demo, in the docs — 2026-08-05. The name moved to the planted corpus the free run actually uses; the v1 thirteen were retired, since nothing loaded them. `/tools` is a website change and is not in this repository
- [x] First domain corpus shipped with a template; **second corpus timed at 4m43s with no structural rework** — 2026-08-05. The timing is an agent's, not a human's; what it establishes is that no design work remained, not that a person would take half a day
- [ ] Hero benchmark run sheet published before the run; aggregate result names nobody
- [x] Licensed-content reproduction shipped, with the three-way split and the "presence is not a breach" limit line printed with every instance — 2026-08-05
- [x] `serve_licensed_content` profile added to the reference target; sensitivity gate now **20/20** — `point_in_time` is registered too, so the registry-driven gate demanded a profile for it as well (2026-08-05)
...
| 4 | ~~`upload` effectively required — a larger access ask than a chat probe~~ **Closed 2026-08-05.** `endpoints.upload` is optional; `mode: existing` reads no path and uploads nothing, and the existing-corpus config declares no upload endpoint at all | High | F25 / Phase G |
...
| 10 | ~~Bundled corpus positioned as the audit rather than the demo~~ **Closed 2026-08-05.** Three corpora ship, each with a README stating what a run of it does not establish; the demo's names the published seed as a disqualifier for engagement use. A corpus is data validated against a spine, so the structural claims are checked rather than described | Medium | §9.4 / Phase H |
...
| 13 | **The `score` lockfile pulls the whole CUDA stack on Linux.** 18 of its 68 packages are `nvidia-*`, `cuda-*` or `triton`, gated on `sys_platform == 'linux'`, on a scoring path §5.3 describes as `torch` (CPU) and §5.4 as CPU-only. `nvidia-cublas` alone is a 543 MB wheel. This is a property of the lockfile, not of the container — every Linux `pip install -r requirements/score.txt` has carried it — but the image is what made it visible and what makes it expensive. Not fixed in the container phase: the fix is a CPU-wheel index in `requirements/score.in`, which changes the lockfile, all four SBOMs, `check_pins.py` and what every Linux user gets, and that is a dependency decision rather than a Dockerfile one | Medium | Open — needs a decision, see §20.1 #9 |
| 12 | ~~No authorisation gating on injection/canary families~~ **Closed 2026-08-06.** Every family both batteries ask is classified as data; uploading is an authorised act in its own right; production needs a second, typed act; the block is reproduced verbatim in the report | High | §13 / Phase I |
...
| 5 | ~~**Retention of returned `responses.jsonl`**~~ **Settled 2026-08-06** | Before the first run | §15.7 — 90 days post-delivery, excerpts retained with the report, no publication without written consent. Written up in `docs/authorisation-and-retention.md`; still to land on `/trust` and in the engagement terms, both outside this repository |
...
| 9 | **Does the `score` layer install CPU-only torch?** Today it does not: the universal lockfile resolves the default PyPI torch, which on Linux drags 18 CUDA packages behind it (defect 13). Nothing in the scoring path uses a GPU | Before the first published `score` image | Add `--extra-index-url https://download.pytorch.org/whl/cpu` (or the `torch` CPU variant) to `requirements/score.in` and re-lock. Two things to check before adopting: that `--generate-hashes` still produces a universal lockfile across macOS arm64 and Linux x86_64, and that the CPU wheel is available for every supported platform — a lockfile that silently disagrees per platform is worse than a large image. If it cannot be made universal, resolve per-platform for the image only and say so, rather than quietly shipping two dependency sets under one name |
...
- [ ] SBOM attached; lockfile hash-pinned; tag GPG-signed; both images cosign-signed **by digest**; SLSA provenance published; `IMAGES` lists the digests and verifies
- [ ] `scripts/verify_release.sh <tag>` passes from a clean clone with cosign and gh installed — including section 5, the images
- [ ] README leads with the artefact route (settled in B2: the strongest invocation is the one where none of our code runs on their machine) and links `docs/hardened-run.md` for when it does
